...
and
$$
\xx = (x,y,z)^T \,, \quad \pmb{b} = (b_1,b_2,b_3)^T \,.
$$
Then $f$ can be represented by the quadratic form
$$
f(\xx) = \xx^T A \xx + \pmb{b} \cdot \xx + c \,.
$$
...
Let $\g \colon (a,b) \to \R^3$ be a smooth curve and $\pmb{a} \colon (a,b) \to \R^3$ a vector, such that $\g(t)$ and $\pmb{a}(t)$ are linearly independent for all $t \in (a,b)$. A **ruled surface** is a surface with 
chart
$$
\sss(u,v) = \g(u) + v \pmb{a}(u) \,.
$$
We say that:

- $\g$ is the **base curve**
- The lines $v \mapsto v \pmb{a}(u)$ are the **rulings**
...
A chart for $\SSS$ is
$$
\sss_u = \dg(u) + v \dot{\pmb{a}}(u) \,, \quad
\sss_v = \pmb{a}(u) \,,
$$
with $\dg$ and $\pmb{a}$ linerly independent. Thus $\dg(u) + v \dot{\pmb{a}}(u)$ and $\pmb{a}$ are linearly independent for
...
with
$$
\g(u):= (\cos(u), \cos(u),0 ) \,, \quad \pmb{a} = (0,0,1) \,.
$$
...
$$
\sss = \g(u) + v \pmb{a}(u) \,, \quad u \in (0,2\pi), \, v \in \left( -\frac12, \frac12 \right) \,,
$$
where 
$$
\g(u) = (\cos(u), \sin(u), 0) 
...
$$
\pmb{a} = \left(   -\sin \left( \frac{u}{2} \right) \cos(u),
