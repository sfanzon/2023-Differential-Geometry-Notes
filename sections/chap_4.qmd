...
$$
where $\SSS_1$ and $\SSS_2$ are surfaces. So far we know what a smooth function from $\R^n$ into $\R^m$ is. The idea is to use surface charts to define such $f$.


::: Definition

Let $\SSS_1$ and $\SSS_2$ be surfaces and let 
$$
\sss_1 \colon U_1 \to \sss_1(U) \subseteq \SSS_1 \,, \quad 
\sss_2 \colon U_2 \to \sss_2(U) \subseteq \SSS_2 \,, 
$$
be surface charts for $\SSS_1$ and $\SSS_2$, respectively. Consider a function
$$
f \colon \SSS_1 \to \SSS_2 \,.
$$
We say that:

- $f$ is smooth if the map
$$
(\sss_2^{-1} \circ f \circ \sss_1 )  \colon U_1 \to U_2
$$
is smooth.

- $f$ is a diffeomorphism if smooth if $\sss_2^{-1} \circ f \circ \sss_1$ is a diffeomorphims. 
...
- The function $\sss_2^{-1} \circ f \circ \sss_1$ maps from $\R^2$ into $\R^2$.
