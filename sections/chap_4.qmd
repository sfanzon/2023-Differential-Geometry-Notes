...
![The Möbius strip is a ruled surface with base curve $\g$ and rulings given by rotating vertical lines.](/images/surfaces_ruled_2.png){#fig-mobius-ruled width=70%}
...
charts. We first need to define the coordinates functions $du$ and $dv$ on $T_{\pp} S$.
...
Using a bit of linear algebra, Proposition \ref{proposition-fff-quadratic} becomes very
clear: $I_{\pp}$ is a symmetric bilinear form on the vector space $T_{\pp} \SSS$. 
...
We proceed similarly to what we did we curves. Recall that if $\g$ is a unit speed curve, the curvature of $\g$ is defined as
$$
\kappa (t) = \norm{\ddg(t)} \,.
$$
...
T_{\pp} \SSS = \operatorname{span} \{ \sss_u , \sss_v \} \,,
$$
where $\sss$ is a regular chart of $\SSS$ at $\pp$. Therefore the unit vector 
$$
\NN := \frac{ \sss_u \times \sss_v }{ \norm{\sss_u \times \sss_v } }
$$
is orthogonal to $T_{\pp} \SSS$. Let $(u_0,v_0) \in \R^2$ be the point such that
$$
\sss(u_0,v_0) = \pp \,.
$$
...
$$
is called the **second fundamental form** of $\SSS$. Therefore $\mathcal{F}_2$ measures how much the normal vector $\NN$ changes while moving across $\SSS$. Let us make the definition
of $\mathcal{F}_2$ precise.


::: Definition
### Principal Unit Normal

Let $\SSS$ be a regular surface and $\sss \colon U \to \SSS$ a regular chart. The **principal unit normal** to $\SSS$ is the function
$$
...
Note that $\NN$ is well defined, since 
$$
\sss_u \times \sss_v \neq 0 \,,
$$
being $\sss$ regular.
...
Again, the proof follows by a simple change of basis argument.
