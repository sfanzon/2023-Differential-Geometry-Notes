...
**first fundamental form** of $\SSS$ at $\pp$ is the map
$$
I_{\pp} \colon T_{\pp} \SSS \to \R \,, \quad I_{\pp} (\vv) := \vv \cdot \vv \,.
$$
...
I_{\pp} (\vv) = \| \vv \|^2 \,,
$$
so that $I_{\pp}$ is computing the length of tangent vectors.



::: Remark
### Computing the first fundamental form {#remark-fff}

The first fundamental form can be computed through regular charts. Let
$\sss$ be a chart at $\pp$. Then 
$$
T_{\pp} \SSS = \operatorname{span} \{ \sss_u, \sss_v \} \,.
$$
Therefore a vector $\vv \in T_{\pp} \SSS$ can be written as linear combination
$$
...
We have
\begin{align*}
I_{\pp} (\vv) & = \vv \cdot \vv = \lambda^2 \, \sss_u \cdot \sss_v + 2\lambda \mu \, \sss_u \cdot \sss_v + \mu^2 \, \sss_v \cdot \sss_v \\
...
Let $\SSS$ be a regular surface and $\sss$ be a regular chart. Let
$$
E := \sss_u \cdot \sss_u \,, \quad  F := \sss_u \cdot \sss_v \,, \quad 
G := \sss_v \cdot \sss_v \,.
$$
The **first fundamental form** of $\sss$ is the expression
$$
E \, du^2 + 2F \, du \, dv + G \, dv^2 \,.
$$
This can be represented by the bilinear form
$$
(du , dv)^T \mathcal{F}_1  (du , dv)\,,
$$
where 
$$
\mathcal{F}_1 :=
...
::: Remark
### Scalar product between tangent vectors  {#remark-fff-sp}

Let $\SSS$ be a regular surface and $\sss$ be a chart at $\pp$. For
$\vv, \ww \in T_{\pp}(\SSS)$ we have
$$
...
We have
\begin{align*}
\vv \cdot \ww & = (\lambda_1 \sss_u + \mu_1 \sss_v) \cdot  ( \lambda_2 \sss_u + \mu_2 \sss_v) \\
              & = \lambda_1 \lambda_2 \, \sss_u \cdot \sss_u + 
                 (\lambda_1\mu_2 + \lambda_2 \mu_1 ) \sss_u \cdot \sss_v
                 + \mu_1 \mu_2 \, \sss_v \cdot \sss_v \\
              & = \lambda_1 \lambda_2 \, E + 
                 (\lambda_1\mu_2 + \lambda_2 \mu_1 ) F
                 + \mu_1 \mu_2 \, G \\
              & = (\lambda_1 , \mu_1)^T 
                  \left( 
                  \begin{array}{cc}  
                  ...
                  (\lambda_2 , \mu_2) \,. 
\end{align*}
Therefore the first fundamental form can also be used to compute the scalar product of two tangent vectors.
...
Finally, let us show how the first fundamental form allows to compute the
...
$$
By Remark \ref{remark-fff} (or direct calculation) we get
$$
\norm{\dg(t)}^2 = \dg \cdot \dg =  E \, \dot{u}^2  + 2F \, \dot{u} \dot{v} + G \, \dot{v}^2 \,,
$$
