...
$$
L(\vv_i) = a_{1i} \ww_1 + \ldots + a_{mi} \ww_m = \sum_{k=1}^m a_{ki} \ww_k  \,.
$$
...
\det ( A - \lambda I  ) = 0 \,,
$$
where $I$ is the identity matrix. An **eigenvector** of $A$ with eigenvalue $\lambda$ is a vector $\vv \neq 0$ such that
$$
A \vv = \lambda \vv \,.
$$
...
- $\pp,\mathbf{q} \in \R^3$ be ortoghonal to each other and to $\ww$.
- $\mathbf{a} \in \pi$ be any point in the plane.


If $\xx \in \pi$ then $\xx-\mathbf{a}$ is parallel to the plane and $\pi$ can be
...
Therefore $T_{\pp} \SSS$ is a plane through the origin, no matter where the point $\pp \in \SSS$ is located. When we draw the tangent plane as a plane resting on the surface, see @fig-tangent-plane, we are not drawing $T_{\pp} \SSS$, but rather the plane 
$$
\pp + T_{\pp} \SSS \,,
$$
...
$$
where $\nn = ({\nn}_1,{\nn}_2,{\nn}_3)$.
The equation of the affine tangent plane $\pp + T_{\pp} \SSS$ is given by
$$
{\nn}_1 (x-x_0) + {\nn}_2 (y-x_0) + {\nn}_3 (z-z_0) = 0 \,, \quad 
\forall \, (x,y,z) \in \R^3 \,.
$$
...
For a proof, see the discussion at page $87$ of [@pressley].
...
circle $\g$. This is shown in @fig-mobius-ruled.
...
\norm{\pp - \mathbf{q}} \,.
$$
However this measures the length of the straight segment which connects $\pp$ to $\mathbf{q}$. We are interested in measuring the distance on $\SSS$.
...
to computing the length of tangent vectors. This motivates the definition of first fundamental form.
...
- The first fundamental form of $\SSS$ at $\pp$ is the map obtained by restricting the scalar product of $\R^3$ to $T_{\pp} \SSS$.
...
- The definition of $I_{\pp}$ does not depend on a chosen chart.
...
$$
\mathcal{F}_1 \colon T_{\pp} \SSS \to \R
$$
defined by
$$
\mathcal{F}_1 (\vv) := E \, du^2(\vv) + 2F \, du(\vv) \, dv (\vv)+ G \, dv^2 (\vv) \,,
$${#eq-fff-chart}
...
$$
The quadratic form $\mathcal{F}_1$ is related to $I_{\pp}$ in the following way.
...
\right) \, (du(\vv) , dv(\ww))^T \,,
$$
for all $\vv,\ww \in T_{\pp} \SSS$. In particular, $\mathcal{F}_1$ is the quadratic form associated to the symmetric bilinear form $I_{\pp}$, that is,
$$
\mathcal{F}_1 (\vv) = I_{\pp} (\vv,\vv) \,, \quad \forall \, \vv \in T_{\pp} \SSS \,.
$$
...
& = E \, du (\vv) dv(\ww)  + F \, ( du(\vv) \, dv(\ww) + du(\ww) + dv(\vv) ) \\
& \qquad  + G \,  dv(\vv) dv(\ww) \\
& = (du (\vv), dv(\ww) )  \, 
\left( 
\begin{array}{cc}
...
\right) \, (du(\vv) , dv(\ww))^T \,.
\end{align*}
The fact that $I_{\pp}(\vv,\vv) = \mathcal{F}_1(\vv)$ is now trivial.
...
With a little abuse of notation, we also denote by $\mathcal{F}_1$ the $2 \times 2$ matrix
$$
\mathcal{F}_1 := 
\left(
\begin{array}{cc}
...
$$
\widetilde{\mathcal{F}}_1 = \widetilde{E} \, d\tilde{u}^2 + 2 \widetilde{F} \, d\tilde{u} d\tilde{v} + \widetilde{G} \, d\tilde{v}^2 \,.
$$
...
Moreover the matrices of $\mathcal{F}_1$ and $\widetilde{\mathcal{F}}_1$ are related by
$$
\left(
\begin{array}{cc}
...
$$
\mathcal{F}_1 = E \, du^2 + 2 F\, du \, dv + G \, dv^2 = du^2 + dv^2 \,.  
$$
...
$$
The above means that the coefficients of $\dg$ with respect to the basis $\{\sss_u,\sss_v\}$ are $\dot u, \dot v$, i.e.,
$$
...
1. Prove that the first fundamental form of $\sss$ is
$$
\mathcal{F}_1 = 2 \, du^2 + u^2 \, dv^2 \,.
$$

2. Let $\g (t):= \sss(t,t)$. Show that
$$
...
Consider the smooth curve $\tg := f \circ \g \colon (t_0,t_1) \to \widetilde{\SSS}$. Setting $\pp :=\g(t)$, by definition of differential of a function between surfaces we have
$$
\dtg(t) = df_{\pp} (\dg(t)) \,.
$$
Hence
...
\norm{\dtg(t)}^2 & = \dtg(t) \cdot \dtg(t) \\
               ...
               & = \norm{\dg(t)}^2
\end{align*}
where in the second last inequality we used that $f$ is a local isometry.
...
*Part 2.* Let $\vv \in T_{\pp} \SSS$. Then there exists a curve $\g \colon (-\e,\e) \to \SSS$ such that
$$
\g(0) = \pp \,, \quad \dg(0) = \vv \,.
$$
...
$$
Since the above is true for each $\e>0$, we infer
$$
...
df_{\pp} (\vv) \cdot df_{\pp} (\vv) & = \dtg(0) \cdot \dtg(0) \\
                                    ...
                                    & = \vv \cdot \vv \,.
\end{align*}
As $\vv$ was arbitrary, we showed that 
$$
df_{\pp} (\vv) \cdot df_{\pp} (\vv) = \vv \cdot \vv \,, \quad \forall \, \vv \in T_{\pp}(\SSS) \,.  
$$
Thanks to Remark \ref{remark-local-isometry} we conclude that $f$ is a local isometry.

:::



We have seen that local isometries preserve the length of curves. 
It also happen that they preserve the first fundamental form.
...
\end{align*}
Usingg that $f$ is a local isometry we get
...
Local isometries are maps which preserve the **scalar product** of tangent vectors. We want to consider maps which preserve the **angle** of tangent
...
$$
\mu (\pp) = \scp{\pmb{\alpha}_1}{\pmb{\alpha}_2 } = 0 \,.
$$
...
$$
\widetilde{\mathcal{F}}_1 = \lambda (u,v) \mathcal{F}_1 \,, \quad \forall \, (u,v) \in U \,,
$$
where $\mathcal{F}_1$ and $\widetilde{\mathcal{F}}_1$ are the first fundamental forms of $\sss$ and $\widetilde{\sss}$, respectively.
...
\sss(u,v) := (u,v,0) \,,
$$
and the surface $\widetilde{\SSS}$ with chart
$$
...
$$
\widetilde{\mathcal{F}}_1 = {\sech}^2(u) \, \left(  du^2 + dv^2 \right) \,.
$$
...
> If $f$ was a local isometry, by Theorem \ref{theorem-local-isometry} we would conclude that $\sss$ and $\widetilde{\sss} = f \circ \sss$ have the same first fundamental form. However 
$$
\mathcal{F}_1 = du^2 + dv^2 \neq {\sech}^2(u) \, \left(  du^2 + dv^2 \right) 
= \widetilde{\mathcal{F}}_1  \,.
$$

- $f$ is a conformal map. 

> The first fundamental forms of $\sss$ and $\widetilde{\sss} = f \circ \sss$ satisfy
$$
\widetilde{\mathcal{F}}_1  = \lambda(u,v) \, \mathcal{F}_1  \,, \quad 
\lambda(u,v) := {\sech}(u) \,.
$$
...
::: Definition
### Conformally flat surface and conformal parametrization {#definition-conformally-flat}

Let $\SSS$ be a regular surface and 
$$
\sss \colon U  \to  \SSS
$$ 
be a regular chart of $\SSS$. We say that
$\SSS$ is **conformally flat** and $\sss$ is a **conformal parametrization**
if the first fundamental form of $\sss$ satisfies
$$
...
Definition \ref{definition-conformally-flat} is motivated by the following 
proposition: It states that angles on conformally flat surfaces look like angles on a plane.
...
- There exists a conformal map $f \colon \pi \to \sss(U) \subseteq \SSS$.
...
$$
where $\mathcal{F}_1$ and $\widetilde{\mathcal{F}}_1$ are the first fundamental forms of $\SSS$ and $\p$, respectively. Since $\p$ is a plane,
...
The cylinder $\SSS$ charted by
$$
\sss (u,v) := (\cos(u), \sin(u), v)
$$
is conformally flat, since the first fundamental form of $\sss$ is
$$
\mathcal{F}_1 = du^2 + dv^2 \,.
$$
...
For this part, see Lecure 9 of Will's Notes (not that long) and section 6.4 in Pressley.
In Lecure 10 of Will's Notes there is an application to the theorem
of archimedes. Looks pretty cool.
...
Before talking about curvatures, we need to clarify what we mean by normal
vector to a surface and orientability. Let $\SSS$ be a regular surface and $\pp \in \SSS$. The tangent plane $T_{\pp} \SSS$ passes through the origin. Therefore $T_{\pp} \SSS$ is completely determined by giving a unit vector $\NN$ perpendicular to it:
$$
...
Let $\SSS$ be a regular surface. We say that $\SSS$ is **orientable** if there exists an 
atlas $\mathcal{A} = \{ \sss_i \}_{i \in I}$ with the property: If $\sss_i, \sss_j \in \mathcal{A}$ are such that $\sss_i (U_i) \cap \sss_j (U_j) \neq \emptyset$, then
$$
\det J \Phi > 0 \,,
$$
...
All the surfaces we encountered in these Lecture Notes
are orientable, except for the Möbius band in Example \ref{example-mobius}. Details about the non-orientability of the Möbius band can be found in Example 4.5.3 in [@pressley].
...
Let $\SSS$ be a regular surface. A **standard unit normal** of $\SSS$ is a smooth function
...
2. There exists a standard unit normal $\NN$ to $\SSS$.
...
Note that, even if a surface is orientable, we always have two choices for the sign of the standard unit normal $\NN$. 
...
The expression
$$
\mathcal{F}_2 :=  L \, du^2 +  2 M  \,du dv  + N \, dv^2 
$$
is called the **second fundamental form** of $\SSS$. Therefore $\mathcal{F}_2$ measures how much the surface $\SSS$ deviates from being a plane. Let us make this definition precise.
...
$$
\mathcal{F}_2 \colon T_{\pp} \SSS \to \R
$$
defined by
$$
\mathcal{F}_2 (\vv) := L \, du^2(\vv) + 2M \, du(\vv) \, dv (\vv)+ N \, dv^2 (\vv) \,,
$${#eq-sff-chart}
...
With a little abuse of notation, we also denote by $\mathcal{F}_2$ the $2 \times 2$ matrix
$$
\mathcal{F}_2 = 
\left(
\begin{array}{cc}
...
$$
\mathcal{F}_2 = L \, du^2 + 2M \, du dv + N \, dv^2
$$
...
$$
\widetilde{\mathcal{F}}_2 = \widetilde{L} \, d\tilde{u}^2 + 2 \widetilde{M} \, d\tilde{u} d\tilde{v} + \widetilde{N} \, d\tilde{v}^2 \,.
$$
The matrices of $\mathcal{F}_2$ and $\widetilde{\mathcal{F}}_2$ are related by
$$
\left(
\begin{array}{cc}
...
$$
\mathcal{F}_2 = L \, du^2 + 2 M\, du \, dv + N \, dv^2 = 0 \,.  
$$
...
$$
\mathcal{F}_2 = L \, du^2 + 2 M\, du \, dv + N \, dv^2 = - du^2  \,.  
$$
...
We have seen that a plane and the unit cylinder have the same first fundamental form 
$$
\mathcal{F}_1 = \widetilde{\mathcal{F}}_1 = du^2 + dv^2 \,,
$$
while their second fundamental forms differ: we have
$$
\mathcal{F}_2 = 0 \,, \quad \widetilde{\mathcal{F}}_2 = - du^2   \,,
$$
respectively.
:::
...
Let $\NN \colon \SSS \to \R^3$ be a standard unit normal to $\SSS$, that is, $\NN$ is a smooth map 
such that
$$
\NN(\pp) \perp T_{\pp} \SSS \,, \quad \| \NN(\pp) \| = 1 \,, \quad \forall  \, \pp \in \SSS \,. 
$$
...
$$
where $\sphere^2$ is the unit sphere in $\R^3$.
...
Let $\SSS$ be an orientable surface and $\NN$ a standard unit normal to $\SSS$. The **Gauss map** of 
$\SSS$ is the map 
$$
...
$$
\mathcal{G}_{\SSS} (\pp) = \NN (\pp) = \frac{ \vv \times \ww  }{ \| \vv \times \ww \| } \,,
$$
...
$$
\mathcal{G}_{\SSS} (u,v) = (\cos(u), \sin(u), 0) \,.
$$
...
The second fudamental form of $\SSS$ is a bilinear symmetric map.

> Indeed, $II_{\pp}$ is symmetric because the scalar product is symmetric. Moreover $\WW_{\pp,\SSS}$ is
linear, being the differential of a smooth map. Hence $II_{\pp}$ is bilinear.
...
\left( 
\begin{array}{cc}
II_{\pp} (\sss_u \cdot \sss_u) & II_{\pp} (\sss_u \cdot \sss_v) \\
II_{\pp} (\sss_v \cdot \sss_u) & II_{\pp} (\sss_v \cdot \sss_v) 
\end{array}
\right)  \,,
$$
...
N = \sss_{vv} \cdot \NN \,.
$$
Therefore, the second fundamental form $II_{\pp}$ coincides with the second fundamental form $\mathcal{F}_{2}$ of the chart $\sss$. We prove this statement in the next theorem.
...
Let $\pp \in \sss(U)$. Then
$$
II_{\pp} (\vv,\ww) = (du (\vv), dv(\ww) )  \, 
\left( 
\begin{array}{cc}
L & M \\
M & N 
...
\right) \, (du(\vv) , dv(\ww))^T \,,
$$
for all $\vv,\ww \in T_{\pp} \SSS$. In particular, $\mathcal{F}_2$ is the quadratic form associated to the symmetric bilinear form $II_{\pp}$, that is,
$$
\mathcal{F}_2 (\vv) = {II}_{\pp} (\vv,\vv) \,, \quad \forall \, \vv \in T_{\pp} \SSS \,.
$$
