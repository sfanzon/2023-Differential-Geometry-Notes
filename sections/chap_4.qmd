...
Let $\g \colon (a,b) \to \R^3$ be a smooth curve and $\mathbf{a} \colon (a,b) \to \R^3$ a vector, such that $\g(t)$ and $\mathbf{a}(t)$ are linearly independent for all $t \in (a,b)$. A **ruled surface** is a surface with 
chart
$$
...
to computing the length of tangent vectors. This motivates the definition of first fundemantal form.
...
::: Proposition
### Matrix representation of first fundamental form  {#proposition-fff-sp}

Let $\sss$ be a regular chart of $\SSS$ at $\pp$. Then 
$$
I_{\pp} (\vv, \ww) = \vv \cdot \ww  = \vv^T \mathcal{F}_1 \ww \,, \quad  \forall \, \vv, \, \ww \in T_{\pp} \SSS \,,
$$
where 
$$
\mathcal{F}_1 := 
...
T_{\pp} \SSS = \operatorname{span} \{ \sss_u, \sss_v \} \,,
$$
meaning that $\{\sss_u,\sss_v\}$ is a basis for $T_{\pp} \SSS$. 
Since $I_{\pp}$ is a bilinear symmetric form on $T_{\pp} \SSS$, by we have
that
$$
I_{\pp} (\vv,\ww) = \vv^T M \ww
$$
with 
$$
M := 
...
\right) \,.
$$
Since $\sss_v \cdot \sss_u = \sss_u \cdot \sss_v$, the proof is concluded.

:::


Traditionally, the first fundamental form is written as the quadratic form
$$
\mathcal{F}_1 = E \, du^2 + 2F \, du \, dv + G \, dv^2 \,.
$$
Let us make this notation precise.


::: Definition
### First fundamental form of a chart

Let $\SSS$ be a regular surface and $\sss$ be a regular chart. Consider 
the basis $\{\sss_u,\sss_v\}$ of $T_{\pp}$. Therefore a vector $\vv \in T_{\pp} \SSS$ can be written as linear combination
$$
\vv = \lambda \sss_u + \mu \sss_v
$$
for some $\lambda,\mu \in \R$. Introduce the linear maps
$$
...
Let $\SSS$ be a regular surface and $\sss$ be a regular chart. Then
$$
I_{\pp} (\vv,\ww) = E \, du (\vv) dv(\ww) + F \, ( du(\vv) \, dv(\ww) + du(\ww) + dv(\vv) ) + G \, dv(\vv) dv(\ww) \,
$$
for all $\vv,\ww \in T_{\pp} \SSS$. The quadratic form associated to $I_{\pp}$ is $\mathcal{F}_1$, that is,
$$
...
& = E \, du (\vv) dv(\ww)  + F \, ( du(\vv) \, dv(\ww) + du(\ww) + dv(\vv) ) + G \,  dv(\vv) dv(\ww) \,.
\end{align*}
The fact that $I_{\pp}(\vv,\vv) = \mathcal{F}_1$ is now trivial.

:::


Let us show how the first fundamental form allows to compute the
length of curves on $\SSS$.
...
Let $\mathbf{a}, \pp, \mathbf{q} \in \R^3$ be orthonormal vectors and consider the plane with chart
$$
...
The above example should not be surprising, since distances on a plane
are the same as Euclidean distances, given that straight segments are 
...
> We have
$$
\sss_u = (0,0,1) \,, \quad \sss_v = (-\sin(v),\cos(v), 0 ) 
$$
and therefore 
\begin{align*}
...
> The proof follows from the elementary identity for 
$$
...
$$
By Proposition \ref{proposition-fff-sp} we get
\begin{align*}
