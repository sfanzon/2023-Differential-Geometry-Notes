...
Rather than Cartesian equations, a more useful way of thinking about curves is viewing them as the *path traced out by a moving point*. If $\gamma(t)$ represents the position a point in $\R^n$ at time $t$, the whole curve can be identified by the function
$$
\gamma \ \colon \R \to \R^n \,, \,\,\, \gamma = \gamma(t) \,.
$$
...
- For each $t \in (a,b)$ the quantity $\gamma(t)$ is a vector in $\R^n$.
...
How do we represent $C$ as a **parametrized curve** $\gamma$? We know that the curve is 2D, therefore we need to find a function
$$
\gamma \ \colon  (a,b) \to \R^2
$$
with componenets 
$$
\gamma(t) = (\gamma_1(t),\gamma_2(t)) \, .
$$
The curve $\gamma$ needs to be chosen so that it parametrizes the set $C$, in the sense that 
$$
